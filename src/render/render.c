 ...
 * @deps-requires: render.h, anim.h, layout.h, card_render.h, core/game_state.h,
 *                 core/card.h, raylib.h, rlgl.h
...
static void sync_buttons(const GameState *gs, RenderState *rs)
{
    ...
    if (rs->contract_ui_active) {
        /* Require both 3 cards selected AND a contract chosen */
...
static void draw_phase_passing(const GameState *gs, const RenderState *rs)
{
    ...
    /* Draw contract options if active */
    if (rs->contract_ui_active) {
        const char *ctr_label = "Choose a Contract:";
        int ctr_size = 20;
        int ctr_w = MeasureText(ctr_label, ctr_size);

        /* Position label above first contract button */
        float label_y = (rs->contract_option_count > 0)
                             ? rs->contract_options[0].bounds.y - 28.0f
                             : bc.y - 140.0f;
        DrawText(ctr_label, (int)(bc.x - (float)ctr_w * 0.5f),
...
static void draw_phase_scoring(const GameState *gs, const RenderState *rs)
{
    ...
    if (rs->show_contract_results) {
        int cr_y = table_y + row_h * (NUM_PLAYERS + 1) + 20;
        DrawLine(table_x, cr_y - 8, table_x + col_w * 3, cr_y - 8, GRAY);

        const char *cr_title = "Contracts";
        DrawText(cr_title, table_x, cr_y, 22, LIGHTGRAY);
        cr_y += 30;

        for (int i = 0; i < NUM_PLAYERS; i++) {
            if (rs->contract_result_text[i][0] == '\0') continue;
            Color cr_col = rs->contract_result_success[i] ? GREEN : RED;
            DrawText(rs->contract_result_text[i], table_x, cr_y, 18, cr_col);
            cr_y += 24;
        }
    }
                                 ...
                                 const char *names[], const char *descs[])
{
    ...
    /* Compute layout using the same config as the rest of the renderer */
    Rectangle rects[4];
    layout_contract_options(&DEFAULT_LAYOUT, count, rects);
